...
$query = "UPDATE categories SET cat_status = '{$bulk_options}' WHERE cat_id = {$postValueId}  ";     
...
$result->execute();     
break; case 'delete':      
$query = "DELETE FROM categories WHERE cat_id = {$postValueId}  ";      
$result = $conn->prepare($query);    
$result->execute();    
break;}}}?>
<div class="container" style="font-size:15px;">
<form action="" method='post'>
<h1 class="m-4 text-center"><?php echo WelcomeToCategories ?></h1>
<table class="table table-striped table-dark">
<div class="row">
<div class="col-3">
<select class="form-control" name="bulk_options" id="">
<option selected value=""><?php echo SelectOptions ?></option>
...
<a class="btn btn-primary" href="add_categories.php"><?php echo Addd ?></a> </div></div>   
<thead><tr>
<th><input id="selectAllBoxes" type="checkbox"></th>
                        ...
                        <th><?php echo Date ?></th>
                        <th><?php echo Posts ?></th>
                        <th><?php echo Managementt ?></th>
                        </tr></thead><tbody><?php 

if($_SESSION['user_role']===User){$t= $_SESSION['user_id'];
  $query = "SELECT * FROM categories WHERE cat_user_id = '$t' ORDER BY cat_id DESC";}
       else {
 ...
 foreach($result->fetchall() as $v => $row){
  $cat_id            = $row['cat_id'];
        ...
        <a class='dropdown-item' href='edit_categories.php?edit={$cat_id}'>" . Edit . "</a>
        <a class='dropdown-item text-danger' class='delete_link' rel='$cat_id' href='#' data-toggle='modal' data-target='#myModal$cat_id'>" . Delete . "</a>
        </div>
        </div> </td>";
        echo "</tr>";?>
  <!-- Modal -->
<div class="modal fade <?php echo _align ?>" id="myModal<?=$cat_id?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
