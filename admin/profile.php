...
<h1 class="text-center"><?php  echo WelcomeToProfile ?></h1></br>
 <?php 
        ...
        $error = ['username'=> '','email'=>'','password'=>'','Confirm'=>''];
            ...
            if($email ==''){
                ...
                foreach ($error as $key => $value) {
                    ...
                    unset($error[$key]); } } 
            if(empty($error)){
                $query = "UPDATE users SET username = ?, user_email = ?, sex = ?, user_cv = ?, user_birthday = ?, country= ?, user_image= ? WHERE user_id = ?";
                $result = $conn->prepare($query);    
                $result->execute([$username,$email,$sex,$user_cv,$user_birthday,$country,$image,$id]);    
...
echo "<div class='alert alert-success text-center' role='alert'>operationaccomplishedsuccessfully</div>";}} 
